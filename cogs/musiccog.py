...
class Music(commands.Cog):
    ...
    async def play_music(self, ctx: commands.Context):
        ...
        if len(self.queue[ctx.guild.id]) > 0:
            ...
            self.bot.loop.create_task(self.voice_clients[ctx.guild.id].disconnect())
    ...
    async def add_to_queue(self, ctx: commands.Context, url: str, position: typing.Optional[int] = None):
        ...
        if "&list=" in url:  # not tested yet
            await ctx.send("Added playlist to queue")
            ytdl = yt_dlp.YoutubeDL({'format': 'bestaudio/best', 'noplaylist': False, 'default_search': 'auto'})
            loop = loop or asyncio.get_event_loop()
            data = await loop.run_in_executor(None, lambda: ytdl.extract_info(url, download=False))
            for entry in data['entries']:
                self.queue[ctx.guild.id].append(entry['url'])
                await ctx.send(f"added {entry['url']} to queue")
            return
